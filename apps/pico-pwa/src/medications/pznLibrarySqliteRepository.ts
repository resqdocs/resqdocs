...
const COLS = 'pzn, wirkstoff, label, category, note'
...
export function createPznLibrarySqliteRepository(client: SqlClient): PznSqliteRepository {
  async function pageQuery(offset: number, limit: number, dir: 'asc' | 'desc' = 'asc'): Promise<PznEntry[]> {
    ...
    const rows = await client.query(
      `SELECT ${COLS} FROM ${TABLE} ORDER BY pzn ${ord} LIMIT ? OFFSET ?`,
      [limit, offset],
    )
    return rows.map(toEntry)
  }

  async function setField(pzn: string, col: 'wirkstoff' | 'label' | 'category' | 'note', value: string): Promise<void> {
  ...
  return {
    ...
    page({ offset, limit, dir }) {
      return pageQuery(offset, limit, dir)
    },

    async search(query, { offset, limit }) {
      const q = query.trim()
      if (!q) return pageQuery(offset, limit)
      const pznNeedle = `%${q}%`
      ...
      if (tokens.length === 0) {
        const rows = await client.query(
          `SELECT ${COLS} FROM ${TABLE} WHERE pzn LIKE ? ORDER BY pzn ASC LIMIT ? OFFSET ?`,
      ...
      const rows = await client.query(
        `SELECT ${COLS} FROM ${TABLE} WHERE pzn LIKE ?
         UNION
         SELECT b.pzn, b.wirkstoff, b.label, b.category, b.note
      ...
      await client.run(
        `INSERT INTO ${TABLE} (${COLS}) VALUES (?, ?, ?, ?, ?)
         ON CONFLICT(pzn) DO UPDATE SET
           wirkstoff = excluded.wirkstoff, label = excluded.label,
           category = excluded.category, note = excluded.note`,
        [norm, sanitizeWirkstoff(data.wirkstoff), sanitizeLabel(data.label), sanitizeCategory(data.category), sanitizeNote(data.note)],
      )
    },
